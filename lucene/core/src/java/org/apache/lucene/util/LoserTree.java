...
public final class LoserTree<T> {
  ...
  private LoserTree(int numLeaves, PriorityQueue.LessThan<? super T> lessThan) {
    if (numLeaves < 0) {
  ...
  public static <T> LoserTree<T> usingComparator(int numLeaves, Comparator<? super T> comparator) {
    return new LoserTree<>(numLeaves, (a, b) -> comparator.compare(a, b) < 0);
  }

  /** Creates a loser tree ordered by the provided {@link PriorityQueue.LessThan}. */
  public static <T> LoserTree<T> usingLessThan(
      int numLeaves, PriorityQueue.LessThan<? super T> lessThan) {
    return new LoserTree<>(numLeaves, lessThan);
  }
