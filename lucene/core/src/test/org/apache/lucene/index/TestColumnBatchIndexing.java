...
public class TestColumnBatchIndexing extends LuceneTestCase {
  ...
  public void testLongColumnKindIntPoints() throws IOException {
    for (DocValuesType dvType :
        new DocValuesType[] {
          DocValuesType.NONE, DocValuesType.NUMERIC, DocValuesType.SORTED_NUMERIC
        }) {
      Directory dir = newDirectory();
      IndexWriter w = new IndexWriter(dir, newIndexWriterConfig());
      ...
      if (dvType != DocValuesType.NONE) {
        type.setDocValuesType(dvType);
      }
      type.freeze();

      int[] raw = {Integer.MIN_VALUE, -5, 0, 7, Integer.MAX_VALUE};
      ...
      assertEquals(1, searcher.count(IntPoint.newExactQuery("val", Integer.MIN_VALUE)));
      assertEquals(1, searcher.count(IntPoint.newExactQuery("val", -5)));
      assertEquals(1, searcher.count(IntPoint.newExactQuery("val", 0)));
      assertEquals(1, searcher.count(IntPoint.newExactQuery("val", Integer.MAX_VALUE)));
      assertEquals(3, searcher.count(IntPoint.newRangeQuery("val", -5, 7)));
  ...
  public void testLongColumnKindLongPoints() throws IOException {
    for (DocValuesType dvType :
        new DocValuesType[] {
          DocValuesType.NONE, DocValuesType.NUMERIC, DocValuesType.SORTED_NUMERIC
        }) {
      ...
      w.addBatch(
          simpleBatch(
              raw.length,
              new ArrayLongColumn("val", type, LongColumn.NumericKind.LONG, docIds, raw)));
      ...
      assertEquals(1, searcher.count(LongPoint.newExactQuery("val", Long.MAX_VALUE)));
  ...
  public void testLongColumnKindFloatPoints() throws IOException {
    for (DocValuesType dvType :
        new DocValuesType[] {
          DocValuesType.NONE, DocValuesType.NUMERIC, DocValuesType.SORTED_NUMERIC
        }) {
      ...
      assertEquals(1, searcher.count(FloatPoint.newExactQuery("val", Float.NEGATIVE_INFINITY)));
  ...
  public void testLongColumnKindDoublePoints() throws IOException {
    for (DocValuesType dvType :
        new DocValuesType[] {
          DocValuesType.NONE, DocValuesType.NUMERIC, DocValuesType.SORTED_NUMERIC
        }) {
      ...
      if (dvType == DocValuesType.NUMERIC) {
        ...
        for (int i = 0; i < raw.length; i++) {
          assertEquals(i, dv.nextDoc());
          assertEquals(Double.doubleToRawLongBits(raw[i]), dv.longValue());
        }
      } else if (dvType == DocValuesType.SORTED_NUMERIC) {
        SortedNumericDocValues dv = leaf.getSortedNumericDocValues("val");
        for (int i = 0; i < raw.length; i++) {
      ...
      assertEquals(
          ...
          searcher.count(
              DoublePoint.newRangeQuery(
                  "val", Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY)));
      assertEquals(1, searcher.count(DoublePoint.newExactQuery("val", -1.5d)));
      assertEquals(1, searcher.count(DoublePoint.newExactQuery("val", Double.POSITIVE_INFINITY)));
  ...
  public void testLongColumnPointsWrongWidthRejected() throws IOException {
    // INT kind with 8-byte points declaration should fail validation.
    Directory dir = newDirectory();
    IndexWriter w = new IndexWriter(dir, newIndexWriterConfig());

    FieldType mismatched = new FieldType();
    mismatched.setDimensions(1, Long.BYTES);
    mismatched.freeze();
  ...
  public void testLongColumnPointsTuplesDefaultEncoding() {
    for (LongColumn.NumericKind kind : LongColumn.NumericKind.values()) {
      FieldType type = new FieldType();
      type.setDimensions(1, pointWidth(kind));
      type.freeze();

      long[] values = sampleValuesForKind(kind);
      int[] docIds = new int[values.length];
      for (int i = 0; i < docIds.length; i++) {
        docIds[i] = i;
      }
      ArrayLongColumn col = new ArrayLongColumn("val", type, kind, docIds, values);

      BinaryTupleCursor cursor = col.pointsTuples();
      for (int i = 0; i < values.length; i++) {
        assertEquals("kind=" + kind + " i=" + i, docIds[i], cursor.nextDoc());
        BytesRef got = cursor.binaryValue();
        byte[] expected = expectedSortableBytes(kind, values[i]);
        assertEquals("kind=" + kind + " i=" + i + " length", expected.length, got.length);
        for (int b = 0; b < expected.length; b++) {
          assertEquals(
              "kind=" + kind + " i=" + i + " byte=" + b, expected[b], got.bytes[got.offset + b]);
        }
      }
  ...
  public void testLongColumnPointsValuesDefaultEncoding() {
    for (LongColumn.NumericKind kind : LongColumn.NumericKind.values()) {
      FieldType type = new FieldType();
      type.setDimensions(1, pointWidth(kind));
      type.freeze();
      ...
      long[] values = sampleValuesForKind(kind);
      ArrayDenseLongColumn col = new ArrayDenseLongColumn("val", type, kind, values);

      BinaryValuesCursor cursor = col.pointsValues();
      for (int i = 0; i < values.length; i++) {
        BytesRef got = cursor.nextValues();
        assertNotNull("kind=" + kind + " i=" + i, got);
        byte[] expected = expectedSortableBytes(kind, values[i]);
        assertEquals("kind=" + kind + " i=" + i + " length", expected.length, got.length);
        for (int b = 0; b < expected.length; b++) {
          assertEquals(
      ...
      assertNull(cursor.nextValues());
    }
  }

  public void testLongColumnPointsValuesSparseThrows() {
    FieldType type = new FieldType();
    type.setDimensions(1, Long.BYTES);
    type.freeze();
    ArrayLongColumn sparse =
        new ArrayLongColumn(
            "val", type, LongColumn.NumericKind.LONG, new int[] {0}, new long[] {1L});
    expectThrows(UnsupportedOperationException.class, sparse::pointsValues);
  }

  private static int pointWidth(LongColumn.NumericKind kind) {
    return (kind == LongColumn.NumericKind.INT || kind == LongColumn.NumericKind.FLOAT)
        ? Integer.BYTES
        : Long.BYTES;
  }

  private static long[] sampleValuesForKind(LongColumn.NumericKind kind) {
    return switch (kind) {
      case INT -> new long[] {Integer.MIN_VALUE, -5, 0, 7, Integer.MAX_VALUE};
      case LONG -> new long[] {Long.MIN_VALUE, -100L, 0L, 42L, Long.MAX_VALUE};
      case FLOAT ->
          new long[] {
            Float.floatToRawIntBits(Float.NEGATIVE_INFINITY),
            Float.floatToRawIntBits(-1.5f),
            Float.floatToRawIntBits(0.0f),
            Float.floatToRawIntBits(2.25f),
  ...
  private static byte[] expectedSortableBytes(LongColumn.NumericKind kind, long raw) {
    byte[] out = new byte[pointWidth(kind)];
    switch (kind) {
      case INT -> NumericUtils.intToSortableBytes((int) raw, out, 0);
      case LONG -> NumericUtils.longToSortableBytes(raw, out, 0);
      case FLOAT ->
          NumericUtils.intToSortableBytes(NumericUtils.sortableFloatBits((int) raw), out, 0);
      case DOUBLE -> NumericUtils.longToSortableBytes(NumericUtils.sortableDoubleBits(raw), out, 0);
    }
    return out;
  }
